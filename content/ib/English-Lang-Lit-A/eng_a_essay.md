---
created: 2025-11-27
last-modified: 2025-11-27
title: English A Essay
---
> [!warning] warning
> this note is a stub needs expanding

broad –> specific\
broad: ideas anyone can accept - less objectionable\
e.g.: carol ann duffy’s mrs. sisyphus is based on the greek myth of sisyphus\
specific: an observation about the sounds for e.g.\
someone could object → now you can talk about it

Intro - thesis - the main thing it will be on

each paragraph has a different topic/idea


ending\
start: echo thesis\
specific → broad


PEEL\
PEAR

P - Point - the first sentence of a paragraph states the point of the paragraph
E - Evidence/Example - in literature, usually a quote
E/A - Explanation/Analysis
L/R - Link to thesis / Reader’s reaction

![[structure_of_essay.png]]
